...
public class UpperLayerOpenFlowEndpoint implements IUpperLayerEndpoint {
	private static final Logger logger = LogManager.getLogger();

	private IUpperLayerSocket openFlowHandler = null;
	...
	public void initialize(UpperLayerEndpointManager manager, UpperLayerEndpointConfig cfg) {
		if (this.cfg != null) {
			logger.warn("{} - Prevented duplicate initialization of UpperLayerOpenFlowEndpoint", cfg.name);
			return;
		}

		this.manager = manager;
		this.cfg = cfg;
		this.openFlowHandler = new ULGenericOpenFlowHandler(cfg, this);
			...
			while (!TableVisor.getInstance().isShutdown()) {
				...
				b.handler(new ChannelInitializer<SocketChannel>() {
					...
					public void initChannel(SocketChannel ch) throws Exception {
						ch.pipeline().addLast(
								new TVMsgEncoder(),
								new TVMsgDecoder(OFFactories.getGenericReader()),
								openFlowHandler
						);
					}
				});
